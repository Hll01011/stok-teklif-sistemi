...
(function(){
  ...
  function costTL(p){return !p?0:p.purchaseCurrency==='USD'&&state.usdRate?nQ(p.purchase)*nQ(state.usdRate):nQ(p.purchase)}
  function saleTL(p){return !p?0:p.saleCurrency==='USD'&&state.usdRate?nQ(p.sale)*nQ(state.usdRate):nQ(p.sale)}
  ...
  function refreshProductsUI(){const filter=$('#product-search')?.value||'';if(typeof window.__hisBaseRenderProducts==='function')window.__hisBaseRenderProducts(filter);enhanceProductRows();updateSelectionBar()}
  ...
  function enhanceProductRows(){
    ...
    const rows=[...body.querySelectorAll('tr')],selected=state.stockQuoteSelected||new Set();
    rows.forEach(row=>{
      const code=row.querySelector('td b')?.textContent?.trim(),p=state.products.find(x=>x.code===code);
      if(!p||!row.children.length||row.dataset.stockQuoteEnhanced)return;
      row.dataset.stockQuoteEnhanced='1';
      const first=row.firstElementChild;if(first)first.innerHTML=`<label class="stock-quote-head"><input class="stock-quote-select" type="checkbox" data-stock-product="${escQ(p.id)}" ${selected.has(p.id)?'checked':''}><b>${escQ(p.code)}</b></label>`;
      const last=row.lastElementChild;if(last)last.innerHTML=`<button type="button" class="stock-quote-action" data-stock-add="${escQ(p.id)}">+ Teklife Ekle</button>`;
    });
    const selectAll=$('#stock-quote-select-all');if(selectAll){const visible=rows.map(r=>r.querySelector('.stock-quote-select')).filter(Boolean);selectAll.checked=visible.length>0&&visible.every(x=>x.checked);selectAll.indeterminate=visible.some(x=>x.checked)&&!selectAll.checked}
  }

  function updateSelectionBar(){ensureBar();const count=(state.stockQuoteSelected||new Set()).size;$('#stock-quote-count').textContent=count;$('#stock-quote-bar').classList.toggle('show',count>0)}

  function openStockQuote(){
    const ids=[...(state.stockQuoteSelected||new Set())];if(!ids.length)return toast('Teklif için önce stoktan ürün seç.','bad');
    if(state.demo)return toast('Demo modunda teklif kaydı yapılamaz.','bad');
    const products=ids.map(id=>state.products.find(p=>p.id===id)).filter(Boolean);openQuote();
    state.quoteItems=products.map(p=>{const price=saleTL(p);return {product_id:p.id,code:p.code,name:p.name,quantity:1,unit_price:price,profit_margin:marginFromPrice(p,price),vat_rate:nQ(p.vat)||20,currency:'TRY'}});
    renderQuoteItems();$('#quote-modal-title').textContent=`Stoktan Yeni Teklif · ${products.length} Ürün`;
    document.querySelector('.quote-stock-info')?.remove();$('#quote-form')?.insertAdjacentHTML('afterbegin',`<div class="quote-stock-info"><span>▦</span><span><b>Stoktan seçildi.</b> Miktarları ve teklif fiyatlarını aşağıdaki tablodan doğrudan düzenleyebilirsin.</span></div>`);
    state.stockQuoteSelected=new Set();updateSelectionBar();
  }

  function enhancedRenderQuoteItems(){
    const c=calcQuote();
    const rows=state.quoteItems.map((i,n)=>{const p=state.products.find(x=>x.id===i.product_id),cost=costTL(p),margin=marginFromPrice(p,i.unit_price);i.profit_margin=margin;return `<tr class="quote-items-enhanced"><td><b>${escQ(i.code)}</b><small>${escQ(i.name)}</small><span class="qi-cost">Maliyet: ${moneyQ(cost)}</span></td><td><input class="qi-qty" data-i="${n}" type="number" min="0.01" step="0.01" value="${i.quantity}"></td><td><input class="qi-price" data-price-i="${n}" type="number" min="0" step="0.01" value="${nQ(i.unit_price).toFixed(2)}"></td><td><input class="qi-margin" data-margin-i="${n}" type="number" step="0.1" value="${margin.toFixed(1)}"><span style="font-size:10px;color:#7e8b99">%</span></td><td><input class="qi-net" data-net-i="${n}" type="number" step="0.01" value="${(nQ(i.unit_price)-cost).toFixed(2)}"><span style="font-size:10px;color:#7e8b99">net fark</span></td><td><b>${moneyQ(i.quantity*i.unit_price)}</b></td><td><button type="button" class="text-btn" data-remove-qi="${n}">Sil</button></td></tr>`}).join('')||'<tr><td colspan="7" class="muted">Henüz ürün eklenmedi.</td></tr>';
    $('#q-items').innerHTML=rows;
    const tr=$('#q-items').closest('table')?.querySelector('thead tr');if(tr)tr.innerHTML='<th>Ürün</th><th>Miktar</th><th>Birim Fiyat</th><th>Kâr %</th><th>Net Fark</th><th>Tutar</th><th></th>';
  ...
  async function approveQuote(id){
    const q=state.quotes.find(x=>x.id===id);if(!q)return;if(q.status==='Onaylandı')return toast('Bu teklif zaten onaylanmış.');
    ...
    try{const {data,error}=await sb.rpc('approve_quote_and_issue_stock',{p_quote_id:id});if(error)throw error;await loadData();renderAll();toast(data?.message||'Teklif onaylandı ve stoklar düşüldü.')}catch(err){toast(err.message||'Teklif onaylanamadı.','bad')}
  }

  function enhanceQuoteRows(){
    const body=$('#quotes-body');if(!body)return;[...body.querySelectorAll('tr')].forEach(row=>{if(row.dataset.quoteEnhanced)return;const no=row.querySelector('td b')?.textContent?.trim(),q=state.quotes.find(x=>x.quote_no===no);if(!q)return;row.dataset.quoteEnhanced='1';const last=row.lastElementChild;if(!last)return;if(q.status==='Onaylandı')last.insertAdjacentHTML('beforeend','<span class="quote-approved-badge">✓ Stok işlendi</span>');else if(q.status==='Bekliyor'||q.status==='Taslak')last.insertAdjacentHTML('beforeend',`<button type="button" class="quote-approve-btn" data-approve-quote="${escQ(q.id)}">✓ Onayla & Stoktan Düş</button>`)})
  }

  function install(){
    if(window.__hisStockQuoteInstalled)return;
    if(typeof state==='undefined'||typeof sb==='undefined'||typeof window.renderProducts!=='function'||typeof window.renderQuotes!=='function'||typeof window.renderQuoteItems!=='function')return setTimeout(install,60);
    window.__hisStockQuoteInstalled=true;injectStyle();ensureBar();
    window.__hisBaseRenderProducts=window.renderProducts;window.renderProducts=function(filter=''){window.__hisBaseRenderProducts(filter);enhanceProductRows();updateSelectionBar()};
    window.__hisBaseRenderQuotes=window.renderQuotes;window.renderQuotes=function(filter=''){window.__hisBaseRenderQuotes(filter);enhanceQuoteRows()};
    window.renderQuoteItems=enhancedRenderQuoteItems;

    document.addEventListener('click',e=>{
      const add=e.target.closest('[data-stock-add]');if(add){state.stockQuoteSelected=state.stockQuoteSelected||new Set();state.stockQuoteSelected.add(add.dataset.stockAdd);updateSelectionBar();return}
      const approve=e.target.closest('[data-approve-quote]');if(approve){approveQuote(approve.dataset.approveQuote);return}
      const rem=e.target.closest('[data-remove-qi]');if(rem)setTimeout(()=>renderQuoteItems(),0);
    });
    document.addEventListener('change',e=>{
      if(e.target.matches('#stock-quote-select-all')){state.stockQuoteSelected=state.stockQuoteSelected||new Set();document.querySelectorAll('.stock-quote-select').forEach(cb=>e.target.checked?state.stockQuoteSelected.add(cb.dataset.stockProduct):state.stockQuoteSelected.delete(cb.dataset.stockProduct));refreshProductsUI();return}
    ...
    $('#q-cancel')?.addEventListener('click',()=>document.querySelector('.quote-stock-info')?.remove());$('#quote-close')?.addEventListener('click',()=>document.querySelector('.quote-stock-info')?.remove());
